            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    @if (array_key_exists('errors', $errores) && array_key_exists('nombre', $errores['errors']))
                        <b class="text-danger">Error{{ $errores['errors']['nombre']['0'] }}</b>
                    @endif
                ...
                <div class="form-group">
                    ...
                    @if (array_key_exists('errors', $errores) && array_key_exists('descripcion', $errores['errors']))
                        <b class="text-danger">Error{{ $errores['errors']['descripcion']['0'] }}</b>
                    @endif
                </div>
                <div class="form-group">
                    <label>Clave</label>
                    ...
                    @if (array_key_exists('errors', $errores) && array_key_exists('clave', $errores['errors']))
                        <b class="text-danger">Error{{ $errores['errors']['clave']['0'] }}</b>
                    @endif
                </div>
                <div class="form-group">
                    <label>Area</label>
                    ...
                    @if (array_key_exists('errors', $errores) && array_key_exists('areas_id', $errores['errors']))
                        <b class="text-danger">Error{{ $errores['errors']['areas_id']['0'] }}</b>
                    @endif
                </div>
                <div class="form-group">
                    <label>Estado Global del programa</label>
                    <select wire:model='datos.estados_globales_id' class="form-control">
                        <option>Estado</option>
                        <option>Activo</option>
                        <option>Inactivo</option>
                    </select>
                    @if (array_key_exists('errors', $errores) && array_key_exists('estados_globales_id', $errores['errors']))
                    <b class="text-danger">Error{{ $errores['errors']['estados_globales_id']['0'] }}</b>
                @endif
                </div>
            </div>
